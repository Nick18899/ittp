...
class Parser(argparse.ArgumentParser):
    def __init__(
        self,
        module_name: str = 'cartesiangrid',
        description: str = 'Argument parse process...'
    ):
    ...
    def make_parse(self):
        ...
        self.add_argument(
            '--limits',
            type=list,
            help='Limits to the interpolation',
            default=[(0, 1), (0, 1), (0, 1)]
        )
        self.add_argument(
            '--points',
            type=parse_tuple,
            help='Points for interpolation',
            default=([0.1], [0.5], [0.3])
        )
    ...
    def _load_image(self) -> np.ndarray:
        if self.args.image_path:
            try:
                ...
                logger.info("Using random noise image.")
